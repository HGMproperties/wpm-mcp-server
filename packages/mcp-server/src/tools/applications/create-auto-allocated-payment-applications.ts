...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'post',
  httpPath: '/v1/applications/{applicationId}/autoallocatedpayments',
  operationId: 'ExternalApiApplicationLedgerAutoAllocatedPayments_CreateApplicationAutoAllocatedPayment',
};

export const tool: Tool = {
  name: 'create_auto_allocated_payment_applications',
  description:
  ...
  inputSchema: {
    ...
    properties: {
      ...
      SendEmailReceipt: {
        type: 'boolean',
        description:
          'An indicator for whether to send an email receipt to the payee. If the payee does not have an email address set, no email will be sent.',
      },
      TotalAmount: {
        type: 'number',
...
export const handler = async (client: WpmMcpServer, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await client.applications.createAutoAllocatedPayment(applicationId, body));
};

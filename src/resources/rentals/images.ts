...
export class Images extends APIResource {
   ...
   */
  retrieve(imageID: number, params: ImageRetrieveParams, options?: RequestOptions): APIPromise<RentalImage> {
    const { propertyId } = params;
    return this._client.get(path`/v1/rentals/${propertyId}/images/${imageID}`, options);
  }
   ...
   */
  update(imageID: number, params: ImageUpdateParams, options?: RequestOptions): APIPromise<RentalImage> {
    const { propertyId, ...body } = params;
    return this._client.put(path`/v1/rentals/${propertyId}/images/${imageID}`, { body, ...options });
  }

  /**
   * Retrieves all images for a rental property. Note this endpoint will only return
   * file metadata such as file names and descriptions. To download files make
   * requests to the
   * [Download File](#tag/Rental-Properties/operation/ExternalApiRentalImageDownloadRequests_GetRentalImageDownloadUrlById)
   * endpoint.
   *
   * <h4>Required permission(s):</h4><span class="permissionBlock">Rentals > Rental properties and units</span> - `View`
   */
  list(
    ...
    return this._client.get(path`/v1/rentals/${propertyID}/images`, { query, ...options });
  }
  ...
  delete(imageID: number, params: ImageDeleteParams, options?: RequestOptions): APIPromise<void> {
    const { propertyId } = params;
    return this._client.delete(path`/v1/rentals/${propertyId}/images/${imageID}`, {
      ...options,
  ...
  /**
   * Use this endpoint to create a temporary URL that can be used to download a
   * property image. This URL expires after 5 minutes.
   *
   * <h4>Required permission(s):</h4><span class="permissionBlock">Rentals > Rental properties and units</span> - `View`
   */
  downloadrequests(
    ...
    const { propertyId } = params;
    return this._client.post(path`/v1/rentals/${propertyId}/images/${imageID}/downloadrequests`, options);
  }
  ...
  updateOrder(
    ...
    return this._client.put(path`/v1/rentals/${propertyID}/images/order`, { body, ...options });
  }
  ...
  uploadrequests(
    ...
    return this._client.post(path`/v1/rentals/${propertyID}/images/uploadrequests`, { body, ...options });
  }
  ...
  videolinkrequests(
    ...
    options?: RequestOptions,
  ): APIPromise<RentalImage> {
    return this._client.post(path`/v1/rentals/${propertyID}/images/videolinkrequests`, { body, ...options });
  }
}
...
export interface ListingEntityFile {
  /**
...
export interface VideoLinkRequest {
  /**
...
export type ImageListResponse = Array<RentalImage>;

export type ImageUpdateOrderResponse = Array<RentalImage>;
...
export interface ImageUpdateParams {
  ...
  /**
   * Body param: Indicates whether the image will be shown in listings for this
   * rental.
   */
  ShowInListing: boolean;

...
export class Notes extends APIResource {
   ...
   */
  create(rentalOwnerID: number, body: NoteCreateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    return this._client.post(path`/v1/rentals/owners/${rentalOwnerID}/notes`, { body, ...options });
  }
  ...
  retrieve(noteID: number, params: NoteRetrieveParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    const { rentalOwnerId } = params;
    return this._client.get(path`/v1/rentals/owners/${rentalOwnerId}/notes/${noteID}`, options);
  }
  ...
  update(noteID: number, params: NoteUpdateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    const { rentalOwnerId, ...body } = params;
    return this._client.put(path`/v1/rentals/owners/${rentalOwnerId}/notes/${noteID}`, { body, ...options });
  }
  ...
  list(
    ...
    return this._client.get(path`/v1/rentals/owners/${rentalOwnerID}/notes`, { query, ...options });
  }
}

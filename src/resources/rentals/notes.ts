...
export class Notes extends APIResource {
   ...
   */
  create(propertyID: number, body: NoteCreateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    return this._client.post(path`/v1/rentals/${propertyID}/notes`, { body, ...options });
  }
  ...
  retrieve(noteID: number, params: NoteRetrieveParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.get(path`/v1/rentals/${propertyId}/notes/${noteID}`, options);
  }
  ...
  update(noteID: number, params: NoteUpdateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.put(path`/v1/rentals/${propertyId}/notes/${noteID}`, { body, ...options });
  }
  ...
  list(
    ...
    return this._client.get(path`/v1/rentals/${propertyID}/notes`, { query, ...options });
  }
}

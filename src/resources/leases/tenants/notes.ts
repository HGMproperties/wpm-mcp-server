...
export class Notes extends APIResource {
   ...
   * @example
   * ```ts
   * const note = await client.leases.tenants.notes.create(0, {
   *   Note: 'x',
   * });
   * ```
   */
  create(tenantID: number, body: NoteCreateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    return this._client.post(path`/v1/leases/tenants/${tenantID}/notes`, { body, ...options });
  }
   ...
   * @example
   * ```ts
   * const note = await client.leases.tenants.notes.retrieve(0, {
   *   tenantId: 0,
   * });
   * ```
   */
  retrieve(noteID: number, params: NoteRetrieveParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.get(path`/v1/leases/tenants/${tenantId}/notes/${noteID}`, options);
  }
   ...
   * @example
   * ```ts
   * const note = await client.leases.tenants.notes.update(0, {
   *   tenantId: 0,
   *   Note: 'x',
   * });
   * ```
  ...
  update(noteID: number, params: NoteUpdateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.put(path`/v1/leases/tenants/${tenantId}/notes/${noteID}`, { body, ...options });
  }

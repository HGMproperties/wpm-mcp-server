...
export class Notes extends APIResource {
   ...
   * @example
   * ```ts
   * const note = await client.associations.units.notes.create(
   *   0,
   *   { Note: 'x' },
   * );
   * ```
  ...
  create(unitID: number, body: NoteCreateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    return this._client.post(path`/v1/associations/units/${unitID}/notes`, { body, ...options });
  }
   ...
   * @example
   * ```ts
   * const note = await client.associations.units.notes.retrieve(
   *   0,
   *   { unitId: 0 },
   * );
   * ```
  ...
  retrieve(noteID: number, params: NoteRetrieveParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    const { unitId } = params;
    return this._client.get(path`/v1/associations/units/${unitId}/notes/${noteID}`, options);
  }
   ...
   * @example
   * ```ts
   * const note = await client.associations.units.notes.update(
   *   0,
   *   { unitId: 0, Note: 'x' },
   * );
   * ```
  ...
  update(noteID: number, params: NoteUpdateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.put(path`/v1/associations/units/${unitId}/notes/${noteID}`, { body, ...options });
  }
   ...
   * @example
   * ```ts
   * const notes = await client.associations.units.notes.list(0);
   * ```
   */
  list(
    unitID: number,
    ...
    return this._client.get(path`/v1/associations/units/${unitID}/notes`, { query, ...options });
  }
}

...
export class Notes extends APIResource {
   ...
   * @example
   * ```ts
   * const note = await client.associations.owners.notes.create(
   *   0,
   *   { Note: 'x' },
   * );
   * ```
   */
  create(ownerID: number, body: NoteCreateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    return this._client.post(path`/v1/associations/owners/${ownerID}/notes`, { body, ...options });
  }
   ...
   * ```ts
   * const note =
   *   await client.associations.owners.notes.retrieve(0, {
   *     ownerId: 0,
   *   });
   * ```
   */
  retrieve(noteID: number, params: NoteRetrieveParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.get(path`/v1/associations/owners/${ownerId}/notes/${noteID}`, options);
  }
   ...
   * @example
   * ```ts
   * const note = await client.associations.owners.notes.update(
   *   0,
   *   { ownerId: 0, Note: 'x' },
   * );
   * ```
  ...
  update(noteID: number, params: NoteUpdateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.put(path`/v1/associations/owners/${ownerId}/notes/${noteID}`, { body, ...options });
  }
   ...
   * @example
   * ```ts
   * const notes = await client.associations.owners.notes.list(
   *   0,
   * );
   * ```
   */
  list(
    ...
    return this._client.get(path`/v1/associations/owners/${ownerID}/notes`, { query, ...options });
  }
}

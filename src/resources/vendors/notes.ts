...
export class Notes extends APIResource {
   ...
   */
  create(vendorID: number, body: NoteCreateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    return this._client.post(path`/v1/vendors/${vendorID}/notes`, { body, ...options });
  }
  ...
  retrieve(noteID: number, params: NoteRetrieveParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    ...
    return this._client.get(path`/v1/vendors/${vendorId}/notes/${noteID}`, options);
  }
  ...
  update(noteID: number, params: NoteUpdateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    const { vendorId, ...body } = params;
    return this._client.put(path`/v1/vendors/${vendorId}/notes/${noteID}`, { body, ...options });
  }
  ...
  list(
    ...
    return this._client.get(path`/v1/vendors/${vendorID}/notes`, { query, ...options });
  }
}

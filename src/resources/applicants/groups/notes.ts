...
export class Notes extends APIResource {
  ...
  create(
    ...
    return this._client.post(path`/v1/applicants/groups/${applicantGroupID}/notes`, { body, ...options });
  }
  ...
  retrieve(noteID: number, params: NoteRetrieveParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    const { applicantGroupId } = params;
    return this._client.get(path`/v1/applicants/groups/${applicantGroupId}/notes/${noteID}`, options);
  }
  ...
  update(noteID: number, params: NoteUpdateParams, options?: RequestOptions): APIPromise<NotesAPI.Note> {
    const { applicantGroupId, ...body } = params;
    return this._client.put(path`/v1/applicants/groups/${applicantGroupId}/notes/${noteID}`, {
      body,
      ...options,
    });
  }
  ...
  list(
    ...
    return this._client.get(path`/v1/applicants/groups/${applicantGroupID}/notes`, { query, ...options });
  }
}
